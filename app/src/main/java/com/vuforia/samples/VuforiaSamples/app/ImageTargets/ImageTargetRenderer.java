...
public class ImageTargetRenderer implements GLSurfaceView.Renderer, SampleAppRendererControl
{
    ...
    private void initRendering()
    {
        ...
        //program 객체로부터shader의 'vertexPosition' 멤버에 대한 핸들을 가져옴
        //attribute : vertex shader에서만 사용 가능. vertex별로 다른 값
        vertexHandle = GLES20.glGetAttribLocation(shaderProgramID,
            "vertexPosition");
        textureCoordHandle = GLES20.glGetAttribLocation(shaderProgramID,
            "vertexTexCoord");
        //uniform : rendering하는 동안 변하지 않는 변수. vertex & fragment shader에서 모두 공유하여 사용 가능
        //vertex shader 타입의 객체에 정의된 modelViewProjectionMatrix에 대한 핸들 획득
        mvpMatrixHandle = GLES20.glGetUniformLocation(shaderProgramID,
            "modelViewProjectionMatrix");
        texSampler2DHandle = GLES20.glGetUniformLocation(shaderProgramID,
            "texSampler2D");

        //shape가 정의된 클래스의 인스턴스 생성
        if(!mModelIsLoaded) {
            mTeapot = new Teapot();
    ...
    public void renderFrame(State state, float[] projectionMatrix)
    {
        ...
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++) {
            ...
            //렌더링상태의 일부분으로 program을 추
            // activate the shader program and bind the vertex/normal/tex coords
            GLES20.glUseProgram(shaderProgramID);
            //teapot 렌더링
            if (!mActivity.isExtendedTrackingActive()) {
                //vertex 속성을 Teapot buffer에 저장되어있는 vertex 좌표들로 정의
                GLES20.glVertexAttribPointer(vertexHandle, 3, GLES20.GL_FLOAT,
                        false, 0, mTeapot.getVertices());
                GLES20.glVertexAttribPointer(textureCoordHandle, 2,
                        GLES20.GL_FLOAT, false, 0, mTeapot.getTexCoords());
                //vertex 속성을 활성화 시켜야 렌더링시 반영되어서 그려짐
