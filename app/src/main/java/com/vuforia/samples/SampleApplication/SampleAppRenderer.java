...
public class SampleAppRenderer {
    ...
    public void render()
    {
        ...
        for (int v = 0; v < viewList.getNumViews(); v++)
        {
            ...
            /*
            projection matrix : 그래픽 객체의 좌표를 재계산하여 디바이스 스크린에 올바르게 매핑
             projection transformation : 그래픾 객체가 출력될 GLSurfaceView의 너비와 높이에 기초하여
             그래픽 객체의 좌표를 조정, 디바이스 화면의 방향이 변해도 똑같이 보이는 그래픽 객쳬를 볼 수 있다.
             OpenGL view의 크기가 정해졌을 때 또는 renderer의 onSurfaceChanged 메소드에서 변경되었을 때
             projection transformation이 계산됨
             */
            ...
            /*getEyeDisplayAdjustmentMatrix(int viewID)
